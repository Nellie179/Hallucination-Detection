...
def get_ex_data(model, prompts, labels, batch_size, centroids, sinkhorn, num_selected_data, cls_dist, args):
    ...
    with torch.no_grad():
        with autocast(dtype=torch.float16):
            ...
            selected_indices = compute_entropy(all_embeddings, centroids, pseudo_label, num_selected_data, cls_dist,
                                               args)
...
def compute_entropy(last_token_rep, centroids, pseudo_label, k, cls_dist, args):
    ...
    if len(ce_class_0) < cls0_num or len(ce_class_1) < cls1_num:
...
def update_centroids_ema_hard(centroids, last_token_rep, pseudo_label, args):
    ...
    updated_centroids = F.normalize(args.ema_decay * centroids + (1 - args.ema_decay) * new_centroids_batch, p=2,
                                    dim=-1)
