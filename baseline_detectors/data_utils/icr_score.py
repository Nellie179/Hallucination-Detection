...
class ICRScore:
    def __init__(self, hidden_states, attentions, skew_threshold=3, entropy_threshold=3, core_positions=None, icr_device=None, use_induction_head=False):
    ...
    def _pooling_attn(self, pooling, use_induction_head):
        ...
        for layer_idx in range(len(self.output_attentions)):
            ...
            if induction_heads_this_layer:
                ...
                input_size = self.output_attentions[layer_idx][0].shape[-2:] if self.output_attentions[layer_idx] else (1, 1)
    ...
    def compute_icr(self, top_k, top_p, pooling, attention_uniform, hidden_uniform, use_induction_head):
        ...
        for layer_idx in range(len(self.pooling_attentions)):
            ...
            # =====================================================================
            # 💡 第二处修改：加入 attn_topk.shape[0] 做三方物理对齐护盾
