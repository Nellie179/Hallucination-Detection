...
if __name__ == "__main__":
    ...
    process_dataset(args.input_jsonl, args.output_h5, args.model_name, args.method, 
                    {"trust_remote_code": args.trust_remote_code, "attn_implementation": args.attn_implementation})# /home/zfang1/Data/Lxy/Benchmark/baseline_detectors/data_utils/extract_qa_hidden_states.py
"""
Extract hidden states, LOGPROBS and ICR Scores from Q+A (Universal Version)
权威拼接版：基于 Tensor 物理拼接，确保 Token 绝对对齐并防止索引越界。
已新增 SEP (Semantic Entropy Probes) 和 PRISM (Prompt-guided) 特征提取逻辑。
    ...
    with h5py.File(output_h5, 'a') as f_h5:
        for sample in tqdm(samples, desc=f"Inference: {method}"):
            ...
            try:
                ...
                # 🚀 修改点 1：物理铲除残缺的 H5 Group，绝对杜绝脏数据
                if g_name in f_h5: 
                    del f_h5[g_name]
                    f_h5.flush()
                
                # 🚀 修改点 2：建立死信队列 (DLQ)，失败的 ID 记入小本本
                error_log_path = output_h5 + ".failed_ids.txt"
                with open(error_log_path, 'a', encoding='utf-8') as f_err:
                    f_err.write(f"{sid}\t{e}\n")
                
                # 🚀 修改点 3：OOM 显存急救。遇到爆显存，立刻清空 Cache，防止连环爆炸
                if "out of memory" in error_msg or "oom" in error_msg:
                    print(f"    [!] 🚨 触发 OOM 防御机制：已打入死信队列，强制清空显存，抢救流水线...")
                    torch.cuda.empty_cache()
