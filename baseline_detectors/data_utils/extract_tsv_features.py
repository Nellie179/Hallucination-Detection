...
from baseline_detectors.data_utils.train_utils import get_last_non_padded_token_rep, compute_ot_loss_cos, \
...
class TSVFeatureExtractor:
    def __init__(self, model_name=None, model=None, tokenizer=None, model_kwargs=None):
        if model is None or tokenizer is None:
            raise ValueError("❌ [TSVExtractor] Must pass shared model and tokenizer instances from runner!")

        self.model_name = model_name
        self.model = model
        self.tokenizer = tokenizer
        self.device = next(model.parameters()).device
        print(f"[*] TSVFeatureExtractor has taken over the model, current inference precision: {self.model.dtype}")
    ...
    def train_vector(self, train_jsonl_path, str_layer, epochs=20, batch_size=8, lr=0.005, lam=5.0):
        ...
        with torch.enable_grad():
            for epoch in range(epochs):
                ...
                pbar = tqdm(range(0, len(cur_p), batch_size), desc=f"Train TSV (Layer {str_layer}) Ep {epoch + 1}",
                            leave=False)
                for start in pbar:
                    ...
                    with torch.no_grad():
                        centroids = update_centroids_ema_hard(centroids, last_token_rep.to(self.device),
                                                              b_labels_oh.to(self.device), args)
    ...
    def evaluate_vector(self, eval_jsonl_path, output_jsonl_path, trained_vector, final_centroids, str_layer,
                        batch_size=8, lam=5.0):
