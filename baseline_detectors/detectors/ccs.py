...
class CCSProbe(object):
    def __init__(self, x0, x1, nepochs=1000, ntries=10, lr=1e-3, batch_size=-1, verbose=False, device="cuda",
                 linear=True, weight_decay=0.01, var_normalize=False):
...
class CCSDetector(BaseDetector):
    ...
    def _extract_features(self, accessor: SampleAccessor):
        if not getattr(accessor, "qa_h5_file", None):
            raise ValueError(f"Sample {accessor.sample_id} is missing QA feature file!")
    ...
    def fit(self, train_accessors: List[SampleAccessor]) -> None:
        ...
        with torch.no_grad():
            p0 = self.ccs_probe.best_probe(
                torch.tensor(self.ccs_probe.normalize(x0, True), dtype=torch.float, device=self.device))
            p1 = self.ccs_probe.best_probe(
                torch.tensor(self.ccs_probe.normalize(x1, False), dtype=torch.float, device=self.device))
            avg_conf = 0.5 * (p0 + (1 - p1))
            preds = (avg_conf.cpu().numpy() < 0.5).astype(int)[:, 0]
            self.needs_flip = (preds == y).mean() < 0.5

        self.is_fitted = True
        logger.info(f"[{self.name}] Training complete, direction flip: {self.needs_flip}")
    ...
    def predict_score(self, accessor: SampleAccessor) -> float:
        ...
        try:
            ...
            with torch.no_grad():
                prob0 = self.ccs_probe.best_probe(
                    torch.tensor(p_norm, dtype=torch.float, device=self.device)).cpu().item()
                prob1 = self.ccs_probe.best_probe(
                    torch.tensor(n_norm, dtype=torch.float, device=self.device)).cpu().item()
