...
CONFIG = {
    ...
    # 2. 待测模型与 Prompt 渲染配置
    "target_model": "meta-llama/Llama-3.1-8B-Instruct",  # 替换为你想测试的任意模型
...
def run_pipeline(custom_config=None):
    # 如果外部传了配置就用外部的，否则用文件顶部的默认 CONFIG
    cfg = custom_config if custom_config is not None else CONFIG

    print("=" * 50)
    print("🚀 开始执行端到端幻觉检测流水线 (Universal Schema 版)")
    print(f"📊 数据集: {cfg['dataset_name']} | 样本数: {cfg['max_samples']}")
    print(f"🧠 待测模型: {cfg['target_model']} (Few-shot: {cfg['num_shots']})")
