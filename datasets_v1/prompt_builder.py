...
class LLMPromptBuilder:
    def __init__(
        ...
        print(f"[*] [PromptBuilder] 正在加载 {model_name} 的 Tokenizer 以获取官方 Chat Template...")
        if self.template_kwargs:
            print(f"    - 透传 Template 参数: {self.template_kwargs}")

        self.tokenizer = AutoTokenizer.from_pretrained(model_name)

        if self.tokenizer.chat_template is None:
            print("[!] 警告: 该模型没有官方 Chat Template，已回退至通用格式。")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("🚀 Prompt Builder: 0-Shot 题型路由测试 (仅测试目标题格式化)")
    print("="*70)
    builder.num_shots = 0
    # 抽取选择题和 Agent 题看看长什么样
    for target in [mock_pool[2], mock_pool[5]]:
        print(f"\n\033[1;36m>>> 测试题型: {target['structured_data']['task_type']} <<<\033[0m")
        print(builder.build_prompt(target_item=target, few_shot_pool=mock_pool))

    print("\n" + "="*70)
    print("🚀 Prompt Builder: 2-Shot 动态交叉 Few-shot 测试")
    print("="*70)
    builder.num_shots = 2
    # 将代码题作为当前目标题，引擎应该会自动从池子里抓两道不一样的题作为历史对话 (User -> Assistant)
    target_coding = mock_pool[4]
    print(f"\n\033[1;33m>>> 目标题型: {target_coding['structured_data']['task_type']} (带2条随机上下文) <<<\033[0m")
